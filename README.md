...
# Option B (hosted, needs a Google AI Studio key):
...
Open the local URL Gradio prints. To sanity-check the pipeline without
the UI:
...
If neither Ollama nor `GEMMA_API_KEY` is available, the client falls back
to a `mock` backend so the app still runs end-to-end for structural
testing — clearly labeled as mock output, never presented as a real
triage result.
