...
def _llm_tier_classification(record: SymptomRecord, client: GemmaClient) -> tuple[UrgencyTier, str]:
    ...
    try:
        ...
        # Covers: network/backend failure, malformed JSON, unexpected tier
        # string. Fail soft into ROUTINE-for-human-review rather than
        # crashing -- the rule-based ceiling (_clamp_up_only) still applies
        # on top of whatever this returns, so a red flag is never lost.
        tier = UrgencyTier.ROUTINE
        rationale = f"Fallback: could not get a model classification ({type(e).__name__}); defaulting to ROUTINE for human review."
